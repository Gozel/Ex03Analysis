...
y = gesture_success_rates_condition;

numberOfBars = length(CONDITIONS);
barColorMap = jet(numberOfBars); 
...
% Fancy up the graph.
grid on;
caption = sprintf('Average 2nd Task Performance per Condition');
title(caption, 'FontSize', fontSize);
xlabel('Conditions', 'FontSize', fontSize);
ylabel('2nd Task Performance in %', 'FontSize', fontSize);
% Restore the x tick marks.
%set(gca, 'XTickMode', 'Auto');
